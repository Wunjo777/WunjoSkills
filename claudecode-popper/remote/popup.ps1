...
# Falls back to local WPF popup if send fails and fallback_to_local is true.
...
function Send-Ntfy {
    if ([string]::IsNullOrEmpty($ntfyTopic)) {
        Write-Host "[Claude Code Popper] ntfy.topic not configured. Set in config.json or CLAUDE_NTFY_TOPIC env var."
        return $false
    }
    ...
    "ntfy" {
        ...
        if (-not $sent) {
            if ($fallbackLocal) {
                ...
                Write-Host "[Claude Code Popper] ntfy send failed (server=$ntfyServer topic=$ntfyTopic). Check config."
                exit 1
            }
        }
    }
